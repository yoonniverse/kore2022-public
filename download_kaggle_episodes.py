# https://www.kaggle.com/code/robga/kore-episode-scraper-match-downloader/notebook
...
COMP = 'kore-2022'
MAX_CALLS_PER_DAY = 5200  # Kaggle says don't do more than 3600 per day and 1 per second
LOWEST_SCORE_THRESH = 1700

META = "meta-kaggle/"
MATCH_DIR = 'data/raw/'
Path(MATCH_DIR).mkdir(exist_ok=True)
...
for key, value in sorted(sub_to_score_top.items(), key=lambda kv: kv[1], reverse=True):
    if num_api_calls_today <= MAX_CALLS_PER_DAY:
        ...
        print(
            f'submission={key}, LB={"{:.0f}".format(value)}, matches={len(set(sub_to_episodes[key]))}, still to save={len(remaining)}')
